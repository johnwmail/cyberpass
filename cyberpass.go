...
var ticketID = flag.String("t", "checking", "The ticket ID")
...
func main() {
	...
	for scanner.Scan() {
		...
		if credential != "copy password error" {
			fmt.Printf("%s  ansible_host=%s  ansible_password=\"'%s'\"\n", hostname, ipaddr, credential)
			fmt.Fprintf(inventory, "%s  ansible_host=%s  ansible_password=\"'%s'\"\n", hostname, ipaddr, credential)
		}
	}
